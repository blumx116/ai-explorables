...
var rendererCode = renderer.code.bind(renderer);
...
var rendererCodespan = renderer.codespan.bind(renderer);
renderer.codespan = function (text) {
  var math = mathsExpression(text);
  // console.log("codespan" + math);
  if (math) return math;
...
function parsePost(path) {
  ...
  var [top, body] = str.replace("---\n", "").split("\n---\n");

  var html = body;
  if (!path.includes(".html")) {
    console.log(path);
    html = path.includes("2022-10-14-llm-tricks-of-the-trade.md")
      ? marked(body)
